import { parseBasic, parseObj } from "npm:tiny-ts-parser";
...
function typecheck(t: Term, tyEnv: TypeEnv): Type {
  switch (t.tag) {
    ...
    case "objectGet": {
      ...
      if (objTy.tag !== "Object") throw "object expected";
      const prop = objTy.props.find((prop) => prop.name === t.propName);
      if (!prop) throw `undefined property ${t.propName}`;
      return prop.type;
    }
    default:
...
console.log(
  typecheck(
    parseObj(` const x = { foo: 1, bar: true }; x.foo;
  `),
    {}
  )
);
